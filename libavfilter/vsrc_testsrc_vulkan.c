...
static av_cold int init_filter(AVFilterContext *ctx)
{
    ...
    RET(ff_vk_pipeline_descriptor_set_add(vkctx, &s->pl, shd, desc, 1, 0, 0));
...
static int testsrc_vulkan_config_props(AVFilterLink *outlink)
{
    ...
    if (!s->out_format_string) {
        ...
        if (vkctx->output_format != AV_PIX_FMT_YUV444P)
            return AVERROR(EINVAL);
    }
...
static const AVOption testsrc_vulkan_options[] = {
...
AVFILTER_DEFINE_CLASS(testsrc_vulkan);
...
const AVFilter ff_vsrc_smptehdbars_vulkan = {
    .name           = "smptehdbars_vulkan",
    .description    = NULL_IF_CONFIG_SMALL("Generate SMPTE HD color bars (Vulkan)"),
    ...
    .priv_class     = &testsrc_vulkan_class,

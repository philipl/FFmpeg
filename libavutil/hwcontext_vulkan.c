...
static const struct {
    ...
    { AV_PIX_FMT_P010, { VK_FORMAT_R10X6_UNORM_PACK16, VK_FORMAT_R10X6G10X6_UNORM_2PACK16 } },
    { AV_PIX_FMT_P016, { VK_FORMAT_R16_UNORM, VK_FORMAT_R16G16_UNORM } },
    { AV_PIX_FMT_NV16, { VK_FORMAT_R16_UNORM, VK_FORMAT_R16G16_UNORM } },
...
static int find_device(AVHWDeviceContext *ctx, VulkanDeviceSelection *select)
{
    ...
    if (choice > -1) {
        ...
        memcpy(hwctx->device_uuid, idp[choice].deviceUUID, VK_UUID_SIZE);
    }
    av_free(devices);
    av_free(prop);
    av_free(idp);
                        ...
                        void *create_pnext)
{
    ...
    AVVkFrame *f = av_mallocz(sizeof(*f));
    if (!f) {
...
static inline uint32_t vulkan_fmt_to_drm(VkFormat vkfmt)
{
    for (int i = 0; i < FF_ARRAY_ELEMS(vulkan_drm_format_map); i++)
        if (vulkan_drm_format_map[i].vk_format == vkfmt)
            return vulkan_drm_format_map[i].va_fourcc;
